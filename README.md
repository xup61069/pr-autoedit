...
錄完影片丟進去,自動響度標準化、去冗詞、剪停頓、保護示範音樂、產繁中字幕,
輸出一個**已經剪好**的 Premiere 專案。你只要跳著確認切點,通過就輸出。
剪太兇或太保守?在面板改設定按「重算剪輯」,幾秒出一個新序列。

目標:把單支教學片的剪輯時間從 2~3 小時壓到 5~10 分鐘人工。
...
| 功能 | 說明 |
|------|------|
| 🎙️ 音訊處理 | 響度標準化到 YouTube 標準。降噪預設**不烘進音檔**,交給 Premiere 掛效果、隨時可調可關;要烘死也有選項 |
| 📝 語音轉文字 | 詞級時間戳,整個系統的唯一真相來源。引擎可換(faster-whisper / FunASR);改了引擎、模型或詞庫自動重新辨識,不用清快取 |
| ✂️ 去冗詞 | 「嗯、呃」必刪;「就是、然後」看語境判斷,沒把握的留給你確認 |
| ⏩ 停頓處理 | 三選一:**看畫面決定**(畫面在動快轉帶過、靜止才剪掉,默默示範的畫面不會消失)、一律快轉、一律剪掉。快轉段可自動消音,避免變調尖聲 |
| 🎵 音樂保護 | 沒講話但有聲音的段落(示範音樂、音效)自動偵測、原樣保留 |
| 🤧 雜音剪除 | 咳嗽、清喉嚨、滑鼠聲這類短促聲響直接剪掉,夠長的才當示範音樂保護 |
| 💬 繁中字幕 | 標點感知斷行、簡轉繁(OpenCC),英文術語不被切斷;在 Premiere 剪完後還能依實際時間軸重新對位 |
| 🎬 交回 Premiere | 剪好的 Premiere 專案 + SRT 字幕 + HTML 審閱報告,含審閱標記。也可選「活專案」模式(全保留 + 顏色標籤,進 Premiere 再決定) |
...
- **信心分級**:必刪的不下標記;沒把握的才下,審閱只看這些。
- **聰明的快取**:調剪輯門檻只重算決策、幾秒完成;改了辨識設定
  (引擎/模型/詞庫)自動重新辨識,不會拿舊結果充數。
- **隨時可反悔**:「重算剪輯」永遠產生新序列,舊的不動;降噪掛在
  Premiere 當效果,不烘死在音檔裡。
- **審閱優先**:不直接輸出成品,而是產出剪好的專案讓你把關,誤判可直接改。
- **可插拔、可自訂**:辨識引擎可換,參數集中管理,調成自己的習慣。
...
- **live(活專案)**:所有片段都保留、只切開並上顏色標籤
  ...
  片段數量多,長片在 Premiere 裡可能變卡,所以是備選。
...
- `VOCAB_CATEGORIES` / `CUSTOM_VOCAB` —— 教學類型詞庫 + 你自己的術語,改善辨識最有效的一招。內建十類(motion / PV / 剪輯 / 編曲 / 特效 / 3D / 動畫 / 遊戲 / 程式 / 攝影),面板可自行編輯或新增類型
- `NOISE_TRIM` —— 剪掉咳嗽、清喉嚨、滑鼠聲這類短促雜音(預設開)
...
> **個人設定不會被更新覆蓋**:面板存的設定在 `config/settings_local.json`、
> 自訂詞庫在 `config/vocab_local.json`、設定組合在 `config/presets_local.json`,
> 手動改的可放 `config/settings_local.py`。這些都會蓋過預設值、不進版控。
...
├─ config/
│  └─ settings.py       ★ 所有可調參數(門檻、詞庫、引擎、VST 設定)
│     settings_local.json / .py   你的個人覆寫(不進版控)
│     vocab_local.json   你自訂的教學類型詞庫(不進版控)
│     presets_local.json 你存的設定組合(不進版控)
├─ core/                核心邏輯(已測試,通常不用動)
│  ├─ models.py         資料結構
...
│  ├─ video_probe.py    畫面活動偵測(沒講話時用來判斷該快轉還是剪掉)
...
├─ tests/               測試(先跑這些確認環境沒問題;含不必開 Premiere 的面板測試)
├─ output/              產物(每支影片一個資料夾)
