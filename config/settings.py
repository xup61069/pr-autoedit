...
# 匯入序列後自動幫每個聲音片段掛上降噪效果。
# 為什麼需要:降噪預設「不烘進音檔」(VST_BAKE=False),所以每條新產生的
# 序列都是沒降噪的原始聲音,不掛等於沒降噪 —— 而每次剪完都要你手動按一次
# 「掛降噪」很煩。只有在 VST_BAKE=False 時才會動作(烘進去了就不用掛)。
# 掛不上去也不影響剪輯結果,面板會告訴你改用音軌混音器(更穩的做法)。
AUTO_APPLY_DENOISE = True
