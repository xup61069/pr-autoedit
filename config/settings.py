...
# 「幫目前序列掛降噪」這顆手動按鈕的安全上限:序列片段數超過就拒絕。
#
# 為什麼要有上限:那顆是「每個片段各掛一個」VoiceFX,而 VoiceFX 是 NVIDIA 的
# GPU AI 降噪,每個實例都佔顯示卡記憶體。剪得兇的教學片一條序列動輒
# 1000~1500 個片段,真的掛下去會把 VRAM 吃爆、Premiere 卡死。
#
# 正確做法是整軌掛一個:視窗 > 音軌混音器,A1 軌的效果插槽選 VoiceFX。
# 一個實例涵蓋整條音軌,效果相同、隨時可調,而且不吃 VRAM。
