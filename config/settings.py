...
# 一行至少要這麼多字,才准「因為停頓」而斷行。
#
# 為什麼要有它:教學片常常講到一半就停下來操作滑鼠,那個停頓一超過上面的
# 門檻就斷行,於是「把」「按」「然後你」被單獨留成一行,連「按/下去」都被
# 拆成兩行。實測四支真實影片,這種碎行佔了 24~26%。
# 補上這個下限之後降到 4~5%,而且短句本來就短的內容(「好。」「對。」)
# 不受影響——那是句號造成的斷行,不走這條規則。
SUBTITLE_MIN_CHARS = 8

# 但停頓久到這個秒數就一定斷行,不管那行才幾個字。
# 沒有這道保險的話,一句「好」後面接一段兩分鐘的示範,會被併成同一行字幕
# ——實測真的產生過 96 秒與 140 秒的字幕行。
SUBTITLE_HARD_GAP_SEC = 4.0
...
PRESET_KEYS = [
    ...
    # NOISE_TRIM 要跟音樂保護放在一起:面板上它就是掛在「音樂/音效保護」
    # 底下的子選項(show_if MUSIC_DETECT),漏了它的話套用組合會出現
    # 「音樂保護回到預設、剪不剪咳嗽卻停在上一個組合」這種半吊子狀態。
    "MUSIC_DETECT", "MUSIC_MIN_SEC", "MUSIC_DB_ABOVE_FLOOR", "NOISE_TRIM",
