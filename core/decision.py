...
def trim_quiet_inside(segments: list[Segment],
    ...
    規則:
      ...
      - 挖出來的安靜區在「這一步」一律標成刪除:這些停頓通常只有零點幾秒,
        快轉沒有意義,而且會在 Premiere 產生大量細碎的變速片段(效能地雷)。
      - quiet 需已排序、彼此不重疊(audio_probe 的輸出天然如此)。

    ⚠️ 「一律刪除」只保證到這一步為止。停頓處理方式選 auto 時,後面的
    apply_motion 會再掃一次 reason="silence" 的段落,把其中「夠長
    (>= MOTION_MIN_SEC)而且當下畫面在動」的翻回快轉 —— 那是刻意的:
    你講到一半停 0.6 秒去拉推桿,那半秒的畫面是有內容的,不該消失。
    MOTION_MIN_SEC(0.5 秒)就是擋住「細碎變速片段」那個效能地雷的閘門,
    比它短的微剪段永遠維持刪除。
    (以前這段註解寫「一律刪除、不做快轉」,跟實際行為是反的。)
...
def apply_motion(segments: list[Segment],
                ...
                fps: float) -> list[Segment]:
    """實作 SILENCE_ACTION="auto"(看畫面決定):

        畫面在動   -> 快轉帶過(看得到,但不佔時間)
        畫面靜止   -> 剪掉

    決策引擎只聽聲音,所以你默默示範操作的那幾秒(拉推桿、開選單、比對前後)
    會被當成停頓剪掉 —— 內容真的消失,而且不容易發現。這一步補上畫面資訊。

    ⚠️ 只有 SILENCE_ACTION="auto" 時才可以呼叫這個函式(由 pipeline 把關)。
    以前它是一個獨立開關,不管使用者選了什麼都會把「畫面靜止」的段落改寫成
    delete —— 選了「一律快轉」的人,靜止的停頓照樣被剪掉,而且審閱報告裡
    看不出來(reason 一樣是 silence)。現在它是三選一的其中一種,
    「靜止就剪掉」是這個選項的定義,不再是偷偷蓋掉別人的選擇。
