...
def _sample_frames(video_path: str, sample_fps: float) -> np.ndarray:
...
def frame_diffs(video_path: str, sample_fps: float,
    ...
    if len(frames) < 2:
        ...
        diff = np.abs(frames[1:].astype(np.int16)
                      - frames[:-1].astype(np.int16)).mean(axis=(1, 2))
