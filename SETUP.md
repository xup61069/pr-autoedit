...
交出去的聲音**保持原始錄音**(只做響度標準化),降噪**在 Premiere 裡掛效果**——
隨時可調強度、隨時可關,不會烘死在音檔裡。掛法二選一:
- 面板「剪輯後工具」按「幫目前序列掛降噪」(實驗性,自動掛到每個聲音片段);
- 或最穩的手動法:視窗 > 音軌混音器,在 A1 軌最上面的效果插槽選你的
  降噪外掛(如 VoiceFX),一次搞定、整軌生效。

想回到「先處理好再交付」的舊做法:把 `VST_BAKE` 設為 True(面板進階設定
「把降噪烘進音檔」打勾),並確認 `VST_CHAIN` 指到你的降噪外掛
(多個外掛就依序排:降噪→EQ→壓縮→limiter),重跑時不要加 --skip-audio。
