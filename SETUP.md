# PR剪教學一條龍 —— 安裝與使用說明(Windows + NVIDIA GPU)

這份給「會跑腳本但不常寫程式」的人。照著做,遇到紅字先看最後的「常見錯誤」。
...
node tests/test_panel_layout.js
```
...
- `VOCAB_CATEGORIES` + `CUSTOM_VOCAB`:**改善辨識最有效的一招**。
  選你影片的教學類型(motion/PV/剪輯/編曲/特效/3D/動畫/遊戲/程式/攝影,
  會自動載入該領域術語),再把你常講的頻道名、人名、慣用詞填進
  面板上教學類型正下方的「**我的額外術語**」(`CUSTOM_VOCAB`)
  ——例如 MIDI 才不會被聽成「謎底」。
  這一欄**優先權最高、永遠不會被截斷**,所以真正不能錯的詞放這裡最保險;
  教學類型的詞則是選越多類、越容易因為超出長度上限而被砍掉尾巴。
...
- `NOISE_TRIM`:剪掉短促雜音(預設開)。咳嗽、清喉嚨、滑鼠喀一聲這種
  「有聲音但沒講話」又短的段落直接剪掉;夠長的才當示範音樂保護,
  分界就是 `MUSIC_MIN_SEC`。報告會單獨列出剪掉了哪幾段供你確認。

  > **剪不掉的兩種咳嗽**:一是講話講到一半咳的——咳嗽跟人聲疊在一起,
  > 用音量分不開;二是連續咳很久的(超過 `MUSIC_MIN_SEC`)——長度上跟
  > 示範音樂沒有差別,硬要剪就會誤傷音樂。這兩種要自己在 Premiere 處理。
...
> **VoiceFX 要自己裝**(免費):<https://xaymar.itch.io/voicefx>
> 裝完本工具會自動找到它,不用填路徑。
>
> 為什麼不幫你一起裝好:它是免費但**不可轉發**的專有軟體(而且用了
> Steinberg 的 VST SDK),本專案是 MIT 授權,把它包進來會把授權關係弄亂;
> 它也**只能在 NVIDIA RTX 顯卡上跑**,包進去等於讓用 AMD / Intel 的人
> 下載一個對他們沒用的大檔案。它目前也已經停止開發。
>
> **大部分人不需要它。**預設的 Premiere 內建三件套對教學片口白已經夠用,
> 而且不吃顯示卡記憶體。
